...
## 2026-09-20 CURRENT — Source audit + semantic #124/#128/#129/#130 + installer/deploy #132/#133/#136/#139/#140/#148/#172 + tunnel #151/#153 + Gitea #155/#156 + durability #165 tamamlandı
...
- #155 kapatıldı: Gitea `Running` artık HTTP health yüzeylerine ek olarak gerçek MCP `initialize + tools/list` ve required tool setini doğruluyor. Ordinary status refresh generic protocol probe'un 2 saniyelik TTL cache'ini kullanıyor; tüm non-smoke probe 10 saniyelik absolute budget ile sınırlı ve bu iç timeout gerçek caller cancellation'dan ayrılıyor. `GiteaProtocolReadinessRegression.ps1 -Live` gerçek 8081 endpoint üzerinde **GITEA_PROTOCOL_READINESS_GREEN_LIVE** verdi; aynı koşuda Tray Release **0 warning / 0 error**.
- #156 kapatıldı: Gitea stop terminal gate normal status early-return'ünü kullanmıyor; Gitea/Caddy/MCP/tunnel health yüzeylerini bağımsız doğruluyor. Privileged stop script scheduled-task action'larının exact executable + arguments kimliğinden owned root process'leri ve child ağacını task stopundan önce yakalıyor, stop boyunca yeniden tarıyor ve task Running state ile tracked process'ler birlikte bitmeden dönmüyor. Caddy/Gitea servisleri bounded stop/wait ile kapatılıyor; health/readiness yüzeyleri de kapanmadan `Offline` başarı üretilmiyor. Live read-only task/process identity doğrulandı; gerçek çalışan zincir audit amacıyla durdurulmadı. `GITEA_STOP_CHAIN_TERMINAL_GREEN`; embedded PowerShell parse GREEN; Tray Release **0 warning / 0 error**.
...
- #128 kapatıldı: Roslyn semantic load project/document budgets artık `ProjectLoadProgress` + immediate workspace snapshot ve linked cancellation ile load sırasında uygulanıyor; WorkspaceFailed diagnostic'leri unbounded queue yerine bounded/thread-safe semantic sink'e yazılıyor. Proposal tarafında changed-document limiti source/new-text materializationından önce, changed-character bütçesi ise her physical proposal'da incremental uygulanıyor; linked proposals full new-text kopyası yerine exact edit listesiyle karşılaştırılıyor. Server emergency ceilings de caller değerlerinden bağımsız absolute sınır sağlıyor. `--semantic-resource-bounds-only` beş ayrı zero-mutation budget senaryosunda GREEN; full Source Edit **61/61 GREEN**, Talvora Release **0 warning / 0 error**. Current Roslyn package: Microsoft.CodeAnalysis Workspaces/MSBuild **5.9.0**.
- #129 kapatıldı: semantic graph artık solution/project + loaded regular/additional/analyzer-config documents + workspace metadata references + conventional MSBuild/restore control revisions ve project-root C# source membership ile physical snapshot'a bağlı. Proposal sonrası unchanged state tekrar doğrulanıyor; ilk physical mutation öncesi Source Edit internal commit guard edinip control files'i commit boyunca external write/delete/replace'e kapatıyor, unchanged document revisions + membership'i guard başında ve commit sonunda yeniden doğruluyor. Drift `SEMANTIC_GRAPH_STALE` ile mevcut rollback yoluna giriyor. Fault-injection regression proposal sonrasında yeni default-glob source/reference ekledi ve zero-mutation stale-graph sonucu verdi. Targeted GREEN; full Source Edit **62/62 GREEN**, Talvora Release **0 warning / 0 error**.
- #129 follow-up: project/solution analyzer ve source-generator binary referansları da physical revision snapshot + commit guard kapsamına eklendi. Böylece Roslyn'in loaded graph'ını etkileyen analyzer/generator binary değişimi de stale-graph olarak fail-closed olur. Targeted regression yeniden GREEN; full Source Edit **62/62 GREEN**, Talvora Release **0 warning / 0 error**.
- #130 kapatıldı: semantic rename artık her request için ayrı internal worker kullanıyor ve MSBuild discovery `WorkingDirectory` değerini ilgili solution/project dizininden alıyor. Böylece workspace `global.json` çözümlemesi önceki repository çağrılarındaki process-wide kayıttan etkilenmiyor. Worker sonucu parent'a workspace/symbol/diagnostics/graph ile taşınıyor; `response.Changes` parent tarafta mutable path listesi olarak yeniden bağlanıp graph yeniden doğrulanıyor. İlk sürümde bu bağ kaybolduğu için normal rename kendi `App/Use.cs` değişikliğini false `SEMANTIC_GRAPH_STALE` sayıyordu; düzeltildi. Hedefli solution/toolchain/stale testleri GREEN; full Source Edit **63/63 GREEN**; Release regression build **0 warning / 0 error**.
- #130 live acceptance tamamlandı: clean `e1ff681` HEAD'den canonical installer üretildi; **257,287,951 bytes**, SHA-256 `98D23CB5069F10833FFF4A8011F649AE9927D4DA3C5028C9E7A00A0387863AEA`. Canonical deploy terminal result **0**; ardından service **Running/Automatic**, PID **11004**, `talvora_system_info.sourceCommit=e1ff681b6023b393b164e676a5c8d2b414bc5606`.
- #172 canonical build sırasında bulundu ve kapatıldı: güncel npm scoped `@latest --json` full metadata'yı singleton array zarfında döndürebiliyor; build scripti yalnız object kabul ettiği için geçerli ast-grep 0.45.3 metadata'sını reddediyordu. Tek registry snapshot korunarak sonuç tam-bir-kayıt koleksiyonuna normalize edildi; package identity/license/SHA-512 integrity kontrolleri değişmedi, çoklu kayıt fail-closed. InstallerAstGrepMetadataRegression **5/5 GREEN**.
- #172 live acceptance tamamlandı: unfinished #130 working tree'sini artifact'e karıştırmamak için clean detached `3becb24` worktree'sinden canonical build yapıldı. Installer **257,276,175 bytes**, SHA-256 `E5890BF619D0C7DF5D6744ED85330A9533CD2FCB407800B19ECEAB0CC46D567B`; independent SYSTEM deploy task manifest identity gate'lerini geçerek canlıya aldı. Sonrasında service **Running/Automatic**, `talvora_system_info.sourceCommit=3becb241a243b93ffcf2113344636421f4121179`.
- Son birleşik Source Edit regression **63/63 GREEN**. --source-cache-only **2/2 GREEN**; InstallerAstGrepMetadataRegression **5/5 GREEN**; NativeInstallerSourceRegression **GREEN**; son #130 Release regression build **0 warning / 0 error**.
...
- Bu checkpoint anında **2 OPEN**: #159, #169. Güncel tekil durumlar BUG-AUDIT.md içinde; diğer oturum ilerlerse buradaki sayı checkpoint olarak kalır.
