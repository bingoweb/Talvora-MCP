...
- [x] #189 — SQLite zero-timeout lock wait: `timeoutSeconds=0` provider no-timeout semantiğine düşüyor; query/execute/schema/backup ortak guard ile positive timeout gerektiriyor. İzole locked-DB RED kanıtı + `PASS sqlite-zero-timeout-rejected` ve targeted `--runtime-bounds-only` GREEN; commit/push/canonical live acceptance pending.
